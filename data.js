...
const MAPS = [
  ...
  { id: 'm_mine', zone: 'mountain', emoji: '⛏️', name: '버려진 광산',
    ...
    pool: ['iron_ore', 'crystal', 'cave_moss', 'flint', 'echo_stone'], special: 'sp_starore' },
  { id: 'm_lake', zone: 'mountain', emoji: '🏞️', name: '고요 호수',
    desc: '바람 한 점 없는 산정 호수.',
    pool: ['mist_drop', 'cave_moss', 'snow_bud', 'cloud_moss', 'pine_cone'], special: 'sp_laketear' },
  { id: 'm_rock', zone: 'mountain', emoji: '🗿', name: '흔들 바위산',
    desc: '건드리면 흔들리는 거대한 바위들.',
    pool: ['echo_stone', 'iron_ore', 'cave_moss', 'flint', 'pine_cone'], special: 'sp_rockchip' },
  { id: 'm_cloud', zone: 'mountain', emoji: '☁️', name: '구름모자 산중턱',
    desc: '구름이 모자처럼 걸린 중턱.',
    pool: ['cloud_moss', 'mist_drop', 'snow_bud', 'eagle_feather', 'cave_moss'], special: 'sp_cloudwool' },
  { id: 'm_eyrie', zone: 'mountain', emoji: '🪶', name: '매 둥지 절벽',
    desc: '매들이 둥지를 튼 아찔한 절벽.',
    pool: ['eagle_feather', 'cloud_moss', 'echo_stone', 'pine_cone', 'mist_drop'], special: 'sp_goldfeather' },
  { id: 'm_frost', zone: 'mountain', emoji: '❄️', name: '서리 골짜기',
    desc: '한여름에도 서리가 남는 골짜기.',
    pool: ['snow_bud', 'mist_drop', 'cave_moss', 'cloud_moss', 'crystal'], special: 'sp_everfrost' },
  { id: 'm_echo', zone: 'mountain', emoji: '📣', name: '메아리 협곡',
    desc: '소리가 몇 번이고 되돌아오는 협곡.',
    pool: ['echo_stone', 'iron_ore', 'flint', 'cave_moss', 'pine_cone'], special: 'sp_echogem' },
  { id: 'm_ropeway', zone: 'mountain', emoji: '🚡', name: '낡은 삭도 터',
    desc: '광석을 나르던 삭도의 흔적.',
    pool: ['iron_ore', 'flint', 'echo_stone', 'pine_cone', 'cave_moss'], special: 'sp_rustykey' },
  { id: 'm_observe', zone: 'mountain', emoji: '🔭', name: '별빛 전망대',
    desc: '밤이면 별이 쏟아지는 전망대.',
    pool: ['cloud_moss', 'snow_bud', 'eagle_feather', 'mist_drop', 'crystal'], special: 'sp_meteor' },
  { id: 'm_ash', zone: 'mountain', emoji: '🌋', name: '화산재 능선',
    desc: '따뜻한 재가 쌓인 능선.',
    pool: ['flint', 'iron_ore', 'cave_moss', 'echo_stone', 'pine_cone'], special: 'sp_fireheart' },
  // ── 포근 평야 지대 ──
  { id: 'p_hill', zone: 'plain', emoji: '☀️', name: '햇살 언덕',
    desc: '하루 종일 볕이 드는 언덕.',
    pool: ['herb', 'berry', 'sun_seed', 'wheat', 'dew'], special: 'sp_sunbead' },
  { id: 'p_gourmet', zone: 'plain', emoji: '🍽️', name: '미식가의 들',
    desc: '맛있는 것만 자란다는 들.',
    pool: ['berry', 'honey', 'walnut', 'wheat', 'herb'], special: 'sp_silverspoon' },
  { id: 'p_mirror', zone: 'plain', emoji: '🪞', name: '거울 저수지',
    desc: '하늘을 그대로 비추는 저수지.',
    pool: ['dew', 'wheat', 'thistle', 'herb', 'butter_flower'], special: 'sp_mirrorbit' },
  { id: 'p_walnut', zone: 'plain', emoji: '🌰', name: '호두 마루',
    desc: '호두나무가 줄지어 선 마루.',
    pool: ['walnut', 'wheat', 'thistle', 'berry', 'herb'], special: 'sp_goldwalnut' },
  { id: 'p_windmill', zone: 'plain', emoji: '🌬️', name: '바람개비 밭',
    desc: '바람개비가 끝없이 돌아가는 밭.',
    pool: ['wheat', 'sun_seed', 'thistle', 'clover', 'herb'], special: 'sp_windseed' },
  { id: 'p_bee', zone: 'plain', emoji: '🐝', name: '꿀벌 목장',
    desc: '벌들이 부지런히 오가는 목장.',
    pool: ['honey', 'butter_flower', 'berry', 'thistle', 'herb'], special: 'sp_queenhoney' },
  { id: 'p_sunset', zone: 'plain', emoji: '🌇', name: '노을 밀밭',
    desc: '노을에 물드는 밀밭.',
    pool: ['wheat', 'sun_seed', 'walnut', 'dew', 'berry'], special: 'sp_duskear' },
  { id: 'p_clover', zone: 'plain', emoji: '🍀', name: '네잎 들판',
    desc: '네잎 클로버가 숨어 있는 들판.',
    pool: ['clover', 'herb', 'dew', 'thistle', 'butter_flower'], special: 'sp_fiveleaf' },
  { id: 'p_picnic', zone: 'plain', emoji: '🧺', name: '소풍 바위',
    desc: '소풍 오기 좋은 널찍한 바위.',
    pool: ['berry', 'honey', 'walnut', 'herb', 'wheat'], special: 'sp_lostribbon' },
  { id: 'p_starfield', zone: 'plain', emoji: '✨', name: '별헤는 평지',
    desc: '누워서 별을 세는 평지.',
    pool: ['dew', 'butter_flower', 'clover', 'thistle', 'sun_seed'], special: 'sp_fallenstar' },
  // ── 울창 숲 지대 ──
  { id: 'f_mist', zone: 'forest', emoji: '🌫️', name: '안개 숲 외곽',
    desc: '늘 옅은 안개가 낀 숲의 가장자리.',
    pool: ['night_dew', 'moss_branch', 'fern', 'wild_ivy', 'petal'], special: 'sp_mistseed' },
  { id: 'f_path', zone: 'forest', emoji: '🚶', name: '완만한 숲길',
    desc: '걷기 좋은 완만한 숲길.',
    pool: ['wild_ivy', 'fern', 'moss_branch', 'petal', 'tree_resin'], special: 'sp_lostcompass' },
  { id: 'f_mushroom', zone: 'forest', emoji: '🍄', name: '버섯 마을',
    desc: '버섯이 집처럼 솟은 자리.',
    pool: ['mushroom', 'fern', 'tree_resin', 'moss_branch', 'night_dew'], special: 'sp_capcrown' },
  { id: 'f_firefly', zone: 'forest', emoji: '🐛', name: '반딧불 늪가',
    desc: '밤이면 반딧불이 가득한 늪가.',
    pool: ['firefly', 'night_dew', 'mushroom', 'moss_branch', 'wild_ivy'], special: 'sp_fireflyjar' },
  { id: 'f_owl', zone: 'forest', emoji: '🦉', name: '부엉이 고목',
    desc: '부엉이가 사는 늙은 나무.',
    pool: ['owl_feather', 'spider_silk', 'moss_branch', 'tree_resin', 'wild_ivy'], special: 'sp_owleye' },
  { id: 'f_spider', zone: 'forest', emoji: '🕸️', name: '거미줄 다리',
    desc: '거미줄이 다리처럼 걸린 골짜기.',
    pool: ['spider_silk', 'fern', 'tree_resin', 'moss_branch', 'firefly'], special: 'sp_silverweb' },
  { id: 'f_spirit', zone: 'forest', emoji: '🌳', name: '이끼 정령터',
    desc: '정령이 머문다는 이끼 낀 터.',
    pool: ['moss_branch', 'fern', 'night_dew', 'petal', 'wild_ivy'], special: 'sp_spiritbit' },
  { id: 'f_bush', zone: 'forest', emoji: '🍃', name: '속삭이는 덤불',
    desc: '바람이 지나면 속삭이는 덤불.',
    pool: ['wild_ivy', 'petal', 'fern', 'spider_silk', 'tree_resin'], special: 'sp_whisperbloom' },
  { id: 'f_moon', zone: 'forest', emoji: '🌙', name: '달빛 공터',
    desc: '달빛만 내려앉는 작은 공터.',
    pool: ['night_dew', 'firefly', 'petal', 'moss_branch', 'owl_feather'], special: 'sp_moondust' },
  { id: 'f_door', zone: 'forest', emoji: '🚪', name: '오래된 나무문',
    desc: '숲 한가운데 서 있는 문.',
    pool: ['tree_resin', 'spider_silk', 'fern', 'moss_branch', 'owl_feather'], special: 'sp_mosskey' },
  // ── 황량 황무지 ──
  { id: 'w_swamp', zone: 'waste', emoji: '🐸', name: '청개구리의 늪',
    desc: '청개구리 울음이 그치지 않는 늪.',
    pool: ['frog_egg', 'dry_root', 'black_feather', 'sand_grain', 'salt_crust'], special: 'sp_frogcrown' },
  { id: 'w_dune', zone: 'waste', emoji: '🏜️', name: '까끌 모래 언덕',
    desc: '발이 푹푹 빠지는 모래 언덕.',
    pool: ['sand_grain', 'cactus', 'mirage_shard', 'lizard_scale', 'salt_crust'], special: 'sp_desertglass' },
  { id: 'w_salt', zone: 'waste', emoji: '🧂', name: '소금 평원',
    desc: '하얗게 마른 소금 평원.',
    pool: ['salt_crust', 'sand_grain', 'bone_frag', 'mirage_shard', 'lizard_scale'], special: 'sp_saltrose' },
  { id: 'w_cart', zone: 'waste', emoji: '🛞', name: '부서진 수레길',
    desc: '수레가 다니던 길의 잔해.',
    pool: ['rust_nail', 'sand_grain', 'dry_root', 'bone_frag', 'lizard_scale'], special: 'sp_brokenspoke' },
  { id: 'w_thorn', zone: 'waste', emoji: '🌵', name: '가시덤불 협곡',
    desc: '가시덤불이 빼곡한 협곡.',
    pool: ['cactus', 'dry_root', 'sand_grain', 'lizard_scale', 'frog_egg'], special: 'sp_thorncrown' },
  { id: 'w_bone', zone: 'waste', emoji: '🦴', name: '뼈의 골짜기',
    desc: '오래된 뼈가 흩어진 골짜기.',
    pool: ['bone_frag', 'dry_root', 'sand_grain', 'rust_nail', 'black_feather'], special: 'sp_fossil' },
  { id: 'w_mirage', zone: 'waste', emoji: '🌀', name: '신기루 오아시스',
    desc: '다가가면 사라지는 오아시스.',
    pool: ['mirage_shard', 'salt_crust', 'sand_grain', 'frog_egg', 'bone_frag'], special: 'sp_miragevial' },
  { id: 'w_tower', zone: 'waste', emoji: '🗼', name: '녹슨 철탑',
    desc: '벼락을 맞은 채 서 있는 철탑.',
    pool: ['rust_nail', 'sand_grain', 'salt_crust', 'black_feather', 'bone_frag'], special: 'sp_boltcore' },
  { id: 'w_lizard', zone: 'waste', emoji: '🦎', name: '도마뱀 바위굴',
    desc: '도마뱀들이 볕을 쬐는 바위굴.',
    pool: ['lizard_scale', 'sand_grain', 'cactus', 'dry_root', 'bone_frag'], special: 'sp_goldscale' },
  { id: 'w_crow', zone: 'waste', emoji: '🐦‍⬛', name: '까마귀 언덕',
    desc: '까마귀가 모이는 마른 언덕.',
    pool: ['black_feather', 'rust_nail', 'sand_grain', 'dry_root', 'bone_frag'], special: 'sp_crowtreasure' },
  // ── 반짝 해안 지대 ──
  { id: 's_beach', zone: 'shore', emoji: '🏖️', name: '반짝 모래사장',
    desc: '햇빛에 모래가 반짝이는 해변.',
    pool: ['shell', 'foam', 'starfish', 'sea_glass', 'crab_claw'], special: 'sp_starsand' },
  { id: 's_cave', zone: 'shore', emoji: '🐚', name: '소라 동굴',
    desc: '파도 소리가 울리는 소라 모양 동굴.',
    pool: ['shell', 'coral', 'pearl_bit', 'sea_glass', 'sea_dew'], special: 'sp_singingconch' },
  { id: 's_rock', zone: 'shore', emoji: '🌊', name: '파도 바위',
    desc: '파도가 끊임없이 부딪는 바위.',
    pool: ['foam', 'crab_claw', 'driftwood', 'starfish', 'shell'], special: 'sp_waveheart' },
  { id: 's_light', zone: 'shore', emoji: '🔦', name: '등대 언덕',
    desc: '밤새 불을 밝히는 등대.',
    pool: ['driftwood', 'sea_glass', 'seaweed', 'sea_dew', 'shell'], special: 'sp_beaconember' },
  { id: 's_coral', zone: 'shore', emoji: '🪸', name: '산호 여울',
    desc: '산호가 자라는 얕은 여울.',
    pool: ['coral', 'pearl_bit', 'starfish', 'foam', 'seaweed'], special: 'sp_coralcrown' },
  { id: 's_gull', zone: 'shore', emoji: '🕊️', name: '갈매기 절벽',
    desc: '갈매기가 줄지어 앉은 절벽.',
    pool: ['shell', 'starfish', 'crab_claw', 'seaweed', 'driftwood'], special: 'sp_gullletter' },
  { id: 's_wreck', zone: 'shore', emoji: '⛵', name: '난파선 잔해',
    desc: '모래에 반쯤 묻힌 난파선.',
    pool: ['driftwood', 'crab_claw', 'sea_dew', 'pearl_bit', 'sea_glass'], special: 'sp_captainwatch' },
  { id: 's_foam', zone: 'shore', emoji: '🫧', name: '물거품 웅덩이',
    desc: '물거품이 고이는 웅덩이.',
    pool: ['foam', 'sea_dew', 'sea_glass', 'coral', 'starfish'], special: 'sp_rainbowfoam' },
  { id: 's_pier', zone: 'shore', emoji: '🌅', name: '해질녘 방파제',
    desc: '노을이 내려앉는 방파제.',
    pool: ['shell', 'crab_claw', 'starfish', 'seaweed', 'foam'], special: 'sp_duskshell' },
  { id: 's_mermaid', zone: 'shore', emoji: '🧜', name: '인어의 바위',
    desc: '인어가 앉았다는 바위.',
    pool: ['pearl_bit', 'sea_glass', 'coral', 'sea_dew', 'shell'], special: 'sp_mermaidscale' },
];
...
window.GameData = {
  INGREDIENTS, ZONES, MAPS, SPECIAL_RATE, RECIPES, RECIPE_MAP, SLUDGE, TIERS,
